...
def translate_text(model_id: str, text: str, src: str, tgt: str) -> Optional[str]:    
    config = Config()
    if DEVDEBUG: print('>translate.py/translate_text for', model_id, text, src, tgt)
    ...
    if DEVDEBUG: print('>translate_text:sentence_batch', sentence_batch)
    ...
    if model['pretranslatechain']:
        ...
        if DEVDEBUG: print('>translate_text:Pre-translate/sentence_batch', sentence_batch)
    
    # Preprocess
    for proc in model['preprocessors']:
        sentence_batch = [proc(s) for s in sentence_batch]
        if DEVDEBUG: print('>translate_text:Preprocess/sentence_batch', sentence_batch)
    ...
    if model['translator']:
        ...
        if DEVDEBUG: print('>translate_text:Translate batch /translated_sentence_batch', translated_sentence_batch)
    else:
        translated_sentence_batch = sentence_batch
        if DEVDEBUG: print('>translate_text:else Translate batch /translated_sentence_batch', translated_sentence_batch)
    
    # Postprocess
    tgt_sentences = translated_sentence_batch
    for proc in model['postprocessors']:
        tgt_sentences = [proc(s) for s in tgt_sentences]
    if DEVDEBUG: print('>translate_text:tgt_sentences', tgt_sentences)
    ...
    if model['posttranslatechain']:
        ...
        if DEVDEBUG: print('>translate_text:Post-translate/tgt_sentences', tgt_sentences)
